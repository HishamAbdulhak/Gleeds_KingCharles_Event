...
2. **Admin pages** (`/admin/**`, `/host`): the user logs in at `/admin/login` inside the browser pane; the walkthrough starts once `localStorage.adminToken` is set on the frontend origin.
3. **A Solo Player**: `curl -s -X POST localhost:8080/api/solo -H 'Content-Type: application/json' -d '{"name":"Zed","email":"zed@example.com"}'` returns `gameId` + `sessionToken`. In a second tab on `/play`, set `sessionStorage['seat:<gameId>'] = '<sessionToken>'` (the Seat), then open `/play/<gameId>`; the page connects, says ready and shows the first question.
4. **Live Host updates**: keep `/host` in the first tab while the second plays; the board changes on `GAME_OVER` without a reload.
5. **A Battle lobby**: on `/host` tap New Battle; the PIN is on `/host/<gameId>`. Join from `/join` in a second tab, or `curl -s -X POST localhost:8080/api/games/<pin>/join -H 'Content-Type: application/json' -d '{"name":"Bob","email":"bob@example.com"}'` (the response's `playerId` also goes in `sessionStorage['player:<gameId>']`, which is how the phone finds itself on the Podium); the Host list and the phone's lobby update as each one lands, and a reload of either re-syncs (`ready` → `LOBBY_UPDATE`).
...
7. **Reconnect** (#11): reload `/play/<gameId>` or `/host/<gameId>` mid-question (the Host's roster comes back too). The page comes back on the same question, and the countdown carries on where it was. After answering, the phone comes back on "Locked in…"; after the end, it gets the final screen and the prize proof again. Stop the backend to see the Reconnecting… banner; the page recovers by itself once the backend is back.

**Sizes**: measure text on every Player screen the walkthrough reaches (lobby, question, locked, result, between, Podium) at 360×740, and the Host at 1920×1080; one representative screen proves only itself. Emulate a size to measure, then drive clicks at the pane's own size (a click on an emulated tab can land off target), and `tabs_select` a tab before screenshotting it.
