...
7. **Reconnect** (#11): reload `/play/<gameId>` or `/host/<gameId>` mid-question. The page comes back on the same question, and the countdown carries on where it was. After answering, the phone comes back on "Locked in…"; after the end, it gets the final screen and the prize proof again. Stop the backend to see the Reconnecting… banner; the page recovers by itself once the backend is back.
