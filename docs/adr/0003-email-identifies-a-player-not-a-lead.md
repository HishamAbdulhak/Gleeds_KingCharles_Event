...
- The STOMP wire gains one personal-queue event, sent to every Player of the Game, both Modes, right after `GAME_OVER`: `BEST_SCORE` `{name, score}`, the Player's name in this Game and their email's best Score since the last Reset (null only when a Reset happened mid-Game). The Game topic cannot carry it: a Battle's topic is shared, and each phone gets only its own. `GAME_OVER` is unchanged. Compatibility: additive. A phone built before it falls into its reducer's `default` arm and keeps its final screen; nothing else subscribes to a personal queue.
- A winner who closes the tab loses the screen they would show (the Seat lives in the tab; a refresh gets the screen back, since Reconnect in docs/adr/0004), and with no email visible to staff there is no fallback. The stand should say so ("keep this screen open to claim your prize"). A winner who replays still has a proof when the new Game ends, because the best Score belongs to the email, not to one Game.
