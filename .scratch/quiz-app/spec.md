...
- Server → client, all as `{type, payload}`: Game topic (`LOBBY_UPDATE`, `QUESTION_START` without the correct option, `ANSWER_COUNT`, `REVEAL`, `LEADERBOARD`, `GAME_OVER`), Host topic (`HOST_STATE`), personal queue (`ANSWER_ACK`, `RESULT`, `SYNC`), leaderboard topic (`DAY_LEADERBOARD`).
